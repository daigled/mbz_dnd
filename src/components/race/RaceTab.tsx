...
function RaceTab(props: RaceTabProps) {
    ...
    const raceOptions = availableRaces.map(aR => {
        const assocSubRaces = availableSubraces.filter(aS => aS.raceName === aR.name)
        let mapped = {...aR}
	...
	return (
		<div className={'character-race-tab-wrap'}>
			...
			<select name="character-race-select" value={characterRaceKey} onChange={e => { onChange(e.target.value) }}>
