...
function DiceRollSelection() {
    ...
    const [ availableKeys, setAvailableKeys ] = useState(['a', 'b', 'c', 'd', 'e', 'f'])
    ...
    const makeSelection = (abilityKey, dieKey) => {
        ...
        if( dieKey === -1 ) {
            ...
            const targetKey = Object.keys(scores).find(k => scores[k] === dieKey)
            updatedScores = {
                ...scores,
                [targetKey]: -1,
                [abilityKey]: dieKey
    ...
    return <div style={{marginTop: '30px', borderTop: 'solid 1px pink'}}>
        <h1
        style={{
            marginBottom: '30px',
            padding: '10px 0',
            borderBottom: 'solid 1px #dedede',
        }}>Dice Roll Selection</h1>
        ...
        <div
            ...
            <div className="scores-summary">
                <h2>Scores</h2>
                <ul>{Object.keys(scores).map(el => (<li><span>{el}:</span> <span>{diceValues.find(die => die.key === scores[el])?.value}</span></li>))}</ul>
            </div>
        </div>
    </div>
}
