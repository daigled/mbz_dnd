...
function ClassTab() {
    ...
    const classOptions = Object.keys(classModules).reduce((acc, curr) => {
        ...
        const targetData = classModules[curr]().then(
            data => {
    ...
    return <div className="class-tab">
        <h2>Select A Class:</h2>
        <select name='class-select' value={selectedClass} onChange={e => setSelectedClass(e.target.value)}>
            {options.map(opt => <option value={opt[0].name}>{opt[0].name}</option>)}
        </select>
    </div>
}
